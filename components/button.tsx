...
const Button = ({ className, href, label }: { href: string; label: string; className?: string }) => {
	return (
		...
		<Link
			href={href}
			className={cn(
				'rounded-sm px-8 py-2.5 overflow-hidden relative group cursor-pointer border-2 font-semibold border-secondary-400 text-white bg-secondary-400 uppercase',
				className
			)}>
